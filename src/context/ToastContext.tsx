        ...
        {toasts.map((toast) => (
          <div
            ...
            className={`pointer-events-auto flex items-center justify-between p-4 rounded-xl border backdrop-blur-xl shadow-2xl transition-all duration-300 animate-slide-up ${
              toast.type === 'success'
                ? 'bg-[#0E2E1D]/90 border-emerald-500/30 text-emerald-200'
                : toast.type === 'error'
                ? 'bg-[#2E0F12]/90 border-red-500/30 text-red-200'
                : 'bg-[#111114]/90 border-white/10 text-white'
            }`}
          >
            <div className="flex items-center gap-3 pr-2">
              {toast.type === 'success' && <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0" />}
              {toast.type === 'error' && <AlertCircle className="w-5 h-5 text-red-400 shrink-0" />}
              {toast.type === 'info' && <Info className="w-5 h-5 text-[#00D6FF] shrink-0" />}
